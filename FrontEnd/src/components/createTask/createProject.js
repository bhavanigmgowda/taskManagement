...
export class createProject extends Component {
    ...
    create(e) {
        ...
        Axios.post('http://localhost:8080/create-project?=count='+this.state.i,'&bean',this.state
        ).then((response) => {
                      ...
                      {/*               <FormControl type="text" name="search"  
                                onChange={(event) => { this.setState({ search: event.target.value }) }}
                                   value={this.state.search} onKeyDown={(event) => { this.searchByEnter(event) }}  placeholder="Search" className="w-55" /> */}
                                        ...
                                        {this.state.show?addfeild(this.state.user,this.state.i):null}
...
function addfeild(email,i){
    return(
        <div>
    <input className="form-control"  type="email"  title="Enter Email" placeholder={email} />
    <div>{i}</div>
    </div>
    )

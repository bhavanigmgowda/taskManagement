...
class Projectmembers extends React.Component {
    ...
    getMembers() {
        ...
        if (JSON.parse(window.localStorage.getItem('isValid'))) {
            Axios.get('http://localhost:8080/get-members?groupId=' + 100)
                .then((response) => {
    ...
    change() {
        ...
        Axios.delete('http://localhost:8080/remove-user-from-project?groupId=' + 100 + '&newEmail=' + this.state.nun + '&removeEmail=' + this.state.cun).then((response) => {
            if (response.data.statusCode === 201) {
                console.log("jkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")
                this.setState({

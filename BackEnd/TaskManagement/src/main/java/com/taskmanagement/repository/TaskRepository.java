...
public interface TaskRepository extends JpaRepository<CreateTaskBean, Integer> {
	...
	@Query(value = "select * from assign_task t where " + " t.project_id=:id", nativeQuery = true)
	...
	@Query(value = "select * from assign_task where   project_id=:pid and status='completed' ",nativeQuery = true)
	List<CreateTaskBean> findCompletedForProject(int pid);
	
	@Query(value = "select * from assign_task where completed>=:dateFrom and project_id=:pid and status='completed' ",nativeQuery = true)
